  ...
  try {
    ...
    if (supabaseError) {
      ...
      if (typeof message === 'string' && message.trim() !== '') {
        console.error(`Supabase error fetching glider flight ${flightId}: ${message}. Full error object:`, supabaseError);
      } else {
        // Log a generic message if specific message is not available but an error object exists
        console.error(`Supabase error (no specific message) fetching glider flight ${flightId}. Error object:`, supabaseError);
      }
    ...
    console.error(`Unexpected exception while trying to fetch glider flight ${flightId}: ${errorMessage}. Full exception:`, e);
    return null;
  }
}
